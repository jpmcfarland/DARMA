...
class image_load_empty_test(unittest.TestCase):
    ...
    def test_load_empty(self):
        print(self.__class__.__name__)
        img = image()
        self.assertIsNone(img.data, msg='data array from empty image not None')
        img = image(filename=EMPTY1)
        self.assertIsNone(img.data, msg='data array from dataless FITS not None')
